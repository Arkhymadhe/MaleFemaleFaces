...
class CelebDataset(Dataset):
    def __init__(self, folder="dataset", transform: Callable = None):
        ...
        self.images = list(
            map(lambda x: os.path.join(self.folders[0], x), os.listdir(self.folders[0]))
        ) + list(
            map(lambda x: os.path.join(self.folders[1], x), os.listdir(self.folders[1]))
        )
...
def get_dataloader(dataset: CelebDataset, batch_size: int, collate_fn: Callable = None):
    dataloader = DataLoader(
        dataset, shuffle=True, batch_size=batch_size, collate_fn=collate_fn
    )
    return dataloader
...
def get_transform(stats: Tuple[tuple, tuple] = None, size: int = 64):
    ...
    train_transform = T.Compose([T.Resize(size), T.ToTensor(), T.Normalize(*stats)])
